...
BEGIN
	...
	ON CONFLICT(table_schema, table_name) DO NOTHING RETURNING test_table_id, user_id INTO _test_table_id, _user_id; 
	...
	IF _append OR _new THEN
		...
		IF _append AND _current_user_id!=_user_id AND _current_user_id IS NOT NULL THEN
			return -1;
		END IF;
		UPDATE grape.test_table SET date_updated = current_timestamp;
