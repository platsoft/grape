...
CREATE OR REPLACE FUNCTION grape.current_user_in_role(_role TEXT) RETURNS BOOLEAN AS $$
	SELECT EXISTS (SELECT 1 FROM grape.user_role WHERE user_id=grape.current_user_id()::INTEGER AND role_name=_role::TEXT);
$$ LANGUAGE sql VOLATILE;	
...
CREATE OR REPLACE FUNCTION grape.current_user_in_role(_roles TEXT[]) RETURNS BOOLEAN AS $$
	SELECT EXISTS (SELECT 1 FROM grape.user_role WHERE user_id=grape.current_user_id()::INTEGER AND role_name=ANY(_roles));
$$ LANGUAGE sql VOLATILE;	
