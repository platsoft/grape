...
BEGIN
	...
	-- get the path
	SELECT role_name INTO _path_role FROM grape.access_path WHERE (_check_path ~ regex_path)=TRUE;
	IF NOT FOUND THEN
		-- not found, allow it?
	...
	IF EXISTS (SELECT 1 FROM grape.user_role WHERE user_id=_user_id::INTEGER AND role_name=_path_role::TEXT) THEN
		RETURN (0, _user_id, _session_id)::grape.access_result_type;
	END IF;
